...
import AddCommunityOrderScreen from './AddCommunityOrderScreen';
import CommunityOrderCard from './CommunityOrderCard';
const CommunityHome = ({navigation}: any) => {
  const openAddCommunityOrderSheet: any = useRef(null);
  const openAddCommitmentRef: any = useRef(null);
  const {dismissAll} = useBottomSheetModal();
  const customSnapOpen = (ref: any, index: number) => {
    // close all the ref before opening the new one
    dismissAll();
    openAddCommitmentRef.current?.close();
    openAddCommunityOrderSheet.current?.close();
    openAddCommitmentRef.current.collapse;
    ...
    _communityOrderService
      ...
      .then(res => {
        if (res.statusCode === 200) {
          setCommunityOrders(() => [...res.data]);
          console.log(res.data[1].participants, 'res.data');
        }
      })
      .catch(err => {
        console.log('Community Orders Error', err);
      });
  };
  ...
  const handleCloseSheet = () => {
    console.log('closeSheet from CommunityHome');
    openAddCommitmentRef.current?.close();
    openAddCommunityOrderSheet.current?.close();
  ...
  return (
    <View className="flex-1 bg-gray-100">
      <View className="flex-row justify-between items-center p-4 bg-white rounded-lg shadow-md">
        <Text>Community</Text>
        <TouchableOpacity
          onPress={() => customSnapOpen(openAddCommunityOrderSheet, 2)}
      ...
      <ScrollView
        ...
        {communityOrders.map((order: any, index: number) => (
          <CommunityOrderCard
            key={index}
            order={order}
            setSelectedOrder={setSelectedOrder}
            openAddCommitmentRef={openAddCommitmentRef}
          />
        ))}
      </ScrollView>
      <BottomSheetComp
        title="Create Post"
        ref={openAddCommunityOrderSheet}
        memoArray={['25%', '50%', '96']}>
        <AddCommunityOrderScreen />
      </BottomSheetComp>
      ...
      <BottomSheetComp
        ...
        memoArray={['25%', '60%', '98%']}>
        <AddCommitment order={selectedOrder} closeSheet={handleCloseSheet} />
      </BottomSheetComp>
    </View>
  );
};
